...

{
	...
	//unsigned char imagedata_out_gray[size2][size1];
	...
	for(i=0;i<size2;i++) {
		for(j=0;j<size1;j++) {
			...
			//imagedata_out_gray[i][j]=(unsigned char)(((int)imagedata[i][j][0]*38+(int)imagedata[i][j][1]*75+(int)imagedata[i][j][2]*15)>>7);
		}
	}
	//matrix extension
	for(i=0;i<=size2-1;i++) {
	...
	//cout<<max<<endl; //find the max magnitude for Canny edge detector
	for(i=0;i<size2;i++) {
		for (j=0;j<size1;j++){
			imagedata_gray[i][j]=(int)(255*(image_gradient[i][j]-min)/(max-min));
			//imagedata_out_gray[i][j] = (unsigned char)(255*(image_gradient[i][j]-min)/(max-min)); // problem 1)
		}
	}
	...
	{
		imagedata_out[height[i]][width[i]] = (unsigned char)255; // set 255 to the pixels below threshold
	}
	for(i=boundary;i<size2*size1;i++)
	{
		imagedata_out[height[i]][width[i]] = (unsigned char)0; // set 0 to the pixels above threshold
	}
